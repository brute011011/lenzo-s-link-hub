...
export const ServiceCard: FC<{ name: string; url: string; description: string; status: string; latencyMs: number }> = ({ name, url, description, status, latencyMs }) => {
  ...
  return (
    <a href={url} target="_blank" rel="noopener noreferrer" className="block group">
      <LiquidGlassCard className="p-9 h-full flex flex-col transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] group-hover:translate-y-[-6px]">
        ...
        <div className="mt-auto pt-6 border-t border-black/[0.04] flex justify-between items-center">
          <div className="flex flex-col text-left">
            <span className="text-[9px] font-black text-[#86868B] uppercase tracking-[0.1em] opacity-60">Status</span>
